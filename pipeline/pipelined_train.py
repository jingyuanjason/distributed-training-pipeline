...
def pipelined_train(model, optimizer, x, x_label, x_spec: list[int], split_num, pipeline_stage_this, loss_fn, dtype=None, device=None, pp_group: dist.ProcessGroup = None, dp_group: dist.ProcessGroup = None):
    if pp_group is None:
    ...
    for i in range(warmup_microbatches):
        ...
        inputs.append(x_in)
        x_out: torch.Tensor = model.forward(x_in)
    ...
    for i in range(warmup_microbatches, split_num):
        ...
        x_out_grad: torch.Tensor = outputs.popleft()
        if pipeline_stage_this == pipeline_stage_total - 1:
            grad_input = loss
            (loss / split_num).backward()
        else:
            grad_recv_work.wait()
            x_out_grad.backward(grad_input)
