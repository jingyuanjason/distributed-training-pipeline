...
def async_weight_wrapper_next(next_module: nn.Module, works: list, dtype=torch.float32, FSDP_communication_group: dist.ProcessGroup = None):
    def sync_weight(module: nn.Module, args):
        if FSDP_communication_group is not None:
            group_size = FSDP_communication_group.size()
        else:
            group_size = dist.get_world_size()
        weight_data = next_module.weight.data
        next_module.weight.data_original = weight_data
        next_module.weight.data = torch.empty(group_size * weight_data.shape[0], *weight_data.shape[1:], device=weight_data.device, dtype=dtype)
        work = dist.all_gather_into_tensor(next_module.weight.data, weight_data.to(dtype=dtype), group=FSDP_communication_group, async_op=True)
        works.append(work)
...
def async_weight_wrapper_backward_next(next_module: nn.Module, works: list, dtype=torch.float32, FSDP_communication_group: dist.ProcessGroup = None):
    def sync_weight(module: nn.Module, grad_output=None):
        if FSDP_communication_group is not None:
            group_size = FSDP_communication_group.size()
        else:
...
def wait_prefetched_weight(works):
    def hook(module: nn.Module, args):
        if works:
            works.popleft().wait()
    return hook


def wait_prefetched_weight_backward(works):
    def hook(module: nn.Module, grad_output=None):
        if works:
            works.popleft().wait()
    return hook
    ...
    def __init__(self, module: nn.Module, FSDP_communication_group: dist.ProcessGroup=None, compute_dtype: torch.dtype = torch.float32, moe_compute_dtype: torch.dtype = torch.bfloat16, *, prefetch_weights: bool = True, compile_experts: bool = True):
        ...
        for layer_name, submodule in dict(module.named_modules()).items():
            ...
            if isinstance(submodule, (LinearLayer, EmbeddingLayer)):
                ...
                submodules_linear.append((layer_name, submodule))
        ...
        for i, (layer_name, submodule) in enumerate(submodules_linear):
            layer_compute_dtype = moe_compute_dtype if ".ffn.router" in f".{layer_name}" else compute_dtype
            ...
            if i == 0:
                ...
                submodule.register_forward_pre_hook(wait_prefetched_weight(self.forward_prework))
            if i + 1 < num_layers:
                next_name, next_module = submodules_linear[i + 1]
                next_dtype = moe_compute_dtype if ".ffn.router" in f".{next_name}" else compute_dtype
                submodule.register_forward_pre_hook(
                    async_weight_wrapper_next(next_module, self.forward_prework, next_dtype, FSDP_communication_group)
                )
            submodule.register_forward_hook(free_weight)
            ...
            if i == num_layers - 1:
                ...
                submodule.register_full_backward_pre_hook(wait_prefetched_weight_backward(self.backward_prehook))
            if i - 1 >= 0:
                prev_name, prev_module = submodules_linear[i - 1]
                prev_dtype = moe_compute_dtype if ".ffn.router" in f".{prev_name}" else compute_dtype
                submodule.register_full_backward_pre_hook(
                    async_weight_wrapper_backward_next(prev_module, self.backward_prehook, prev_dtype, FSDP_communication_group)
                )
            submodule.register_full_backward_hook(free_weight_backward)
        self.module = module
