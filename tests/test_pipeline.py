...
from pipeline import pipelined_train, pipelined_train_overlap
...
def test_overlap_preserves_auxiliary_loss_metrics():
    class AuxiliaryStage(PipelineStage):
        def forward(self, x, return_aux_loss=False):
            output = self.layer(x)
            return (output, output.square().mean()) if return_aux_loss else output

    torch.manual_seed(42)
    reference = nn.Linear(4, 4)
    stage = AuxiliaryStage(deepcopy(reference))
    optimizer = torch.optim.SGD(stage.parameters(), lr=0.1)
    reference_optimizer = torch.optim.SGD(reference.parameters(), lr=0.1)
    x, labels = torch.randn(8, 3, 4), torch.randn(8, 3, 4)
    output = reference(x)
    task = nn.functional.mse_loss(output, labels)
    auxiliary = output.square().mean()
    total = task + 0.1 * auxiliary
    total.backward()
    reference_optimizer.step()
    metrics = pipelined_train_overlap(
        stage, optimizer, x, labels, list(x.shape), 4, 0,
        nn.functional.mse_loss, dtype=torch.float32, device="cpu",
        router_aux_loss_coef=0.1, return_metrics=True,
    )
    for name, expected in (("task_loss", task), ("router_aux_loss", auxiliary), ("total_loss", total)):
        torch.testing.assert_close(metrics[name], expected)
