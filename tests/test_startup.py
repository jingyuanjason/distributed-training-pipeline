...
def test_schedule_modules():
    ...
    assert pipelined_train.__module__ == "pipeline.pipelined_train"
    assert pipelined_train_overlap.__module__ == "pipeline.pipelined_train_overlap"
    assert not hasattr(layers, "pipelined_train")
    assert not hasattr(layers, "pipelined_train_overlap")
    assert not (ROOT / "implementation" / "pipeline").exists()
    import distributed_parallel_training_pipelined as training
    assert training.pipelined_train_overlap is pipelined_train_overlap


def test_configs():
    for name in ("smoke", "distributed_training_config"):
